...
impl Ui {
    ...
    fn compute(&mut self) {
        ...
        let _ = taffy.compute_layout_with_measure(root, space, |inputs, _node, node_context, style| {
            taffy::compute_leaf_layout(
                inputs,
                style,
                |_value, _basis| 0.0,
                |known, available| {
                    match node_context.as_deref() {
    ...
    fn update_scroll_metrics(&mut self) { for i in 0..self.scrolls.len() { let v = self.taffy.layout(self.scroll_nodes[i]).ok().copied(); let c = self.taffy.layout(self.scroll_contents[i]).ok().copied(); if let (Some(v), Some(c)) = (v, c) { self.scrolls[i].viewport_height = v.size.height; self.scrolls[i].content_height = c.size.height; self.scrolls[i].set_offset(self.scrolls[i].offset_y); } } }
    fn apply_layout(&mut self) { for i in 0..self.widgets.len() { if let Ok(l) = self.taffy.layout(self.widget_nodes[i]) { self.widgets[i].set_rect(Rect::from_taffy(l)); } } }
    fn dispatch(&mut self, event: &UiEvent, clicked: &mut Vec<WidgetId>) -> bool { match event { UiEvent::Down { x, y } => { if let Some((sid, grab)) = self.scrollbar_at(*x, *y) { if let Some(s) = self.scrolls.get_mut(sid.0) { s.dragging = true; s.drag_grab = grab; } return true; } let target = self.hit_test(*x, *y); if let Some(id) = target { if self.widgets[id.0].focusable() { self.set_focus(Some(id)); } let r = self.widgets[id.0].event(event, self.focus == Some(id)); if r == EventResult::Clicked { clicked.push(id); } return r != EventResult::Ignored; } false }, UiEvent::Move { x, y } => { let mut changed = false; for i in 0..self.scrolls.len() { if self.scrolls[i].dragging { let sid = ScrollViewId(i); let max = self.scrolls[i].max_offset(); if let Ok(v) = self.taffy.layout(self.scroll_nodes[i]) { let track_h = v.size.height.max(1.0); let thumb_h = (track_h * (self.scrolls[i].viewport_height / self.scrolls[i].content_height.max(self.scrolls[i].viewport_height))).max(12.0).min(track_h); let y0 = v.location.y; let rel = (*y as f32 - y0 - self.scrolls[i].drag_grab).max(0.0).min((track_h - thumb_h).max(0.0)); let off = if track_h <= thumb_h { 0.0 } else { rel / (track_h - thumb_h) * max }; changed |= self.scroll_to(sid, off); } } } if let Some(id) = self.hit_test(*x, *y) { let r = self.widgets[id.0].event(event, self.focus == Some(id)); changed |= r != EventResult::Ignored; } changed }, UiEvent::Up { x, y } => { let mut changed = false; for s in &mut self.scrolls { changed |= s.dragging; s.dragging = false; } if let Some(id) = self.hit_test(*x, *y) { let r = self.widgets[id.0].event(event, self.focus == Some(id)); changed |= r != EventResult::Ignored; } changed }, UiEvent::KeyDown { .. } | UiEvent::KeyUp { .. } => { if let Some(id) = self.focus { let r = self.widgets[id.0].event(event, true); if r == EventResult::Submitted { if let Some(cb) = self.clicks.get_mut(id.0) { if cb.is_some() { clicked.push(id); } } } r != EventResult::Ignored } else { false } } }
    }
    fn hit_test(&self, x: i32, y: i32) -> Option<WidgetId> { for i in (0..self.widgets.len()).rev() { let r = self.widgets[i].rect(); if r.contains(x, y) && self.point_visible_in_scroll(r) { return Some(WidgetId(i)); } } None }
    fn point_visible_in_scroll(&self, r: Rect) -> bool { for i in 0..self.scrolls.len() { if let Ok(v) = self.taffy.layout(self.scroll_nodes[i]) { let vr = Rect::new(v.location.x as i32, v.location.y as i32, v.size.width as u32, v.size.height as u32); if r.intersect(vr).is_none() { return false; } } } true }
    fn scrollbar_at(&self, x: i32, y: i32) -> Option<(ScrollViewId, f32)> { for i in (0..self.scrolls.len()).rev() { if let Ok(v) = self.taffy.layout(self.scroll_nodes[i]) { let track_w = 10.0; let xr = v.location.x + v.size.width - track_w; if x as f32 >= xr && x as f32 <= v.location.x + v.size.width && y as f32 >= v.location.y && y as f32 <= v.location.y + v.size.height { let track_h = v.size.height.max(1.0); let thumb_h = (track_h * (self.scrolls[i].viewport_height / self.scrolls[i].content_height.max(self.scrolls[i].viewport_height))).max(12.0).min(track_h); let rel = (y as f32 - v.location.y).max(0.0).min(track_h); let grab = (rel - thumb_h * 0.5).max(0.0).min((track_h - thumb_h).max(0.0)); return Some((ScrollViewId(i), grab)); } } } None }
    fn draw(&self, win: &mut Window) { let _ = Rectangle::new(Point::new(0, 0), Size::new(self.root_w, self.root_h)).into_styled(PrimitiveStyle::with_fill(Rgb888::new(BG.0, BG.1, BG.2))).draw(win); self.draw_node(self.root, win); self.draw_scrollbars(win); }
    fn draw_node(&self, node: NodeId, win: &mut Window) { let Ok(l) = self.taffy.layout(node) else { return }; let nr = Rect::new(l.location.x as i32, l.location.y as i32, l.size.width.max(0.0) as u32, l.size.height.max(0.0) as u32); if let Ok(ctx) = self.taffy.get_node_context(node) { if let Some(NodeCtx::Widget(id)) = ctx { if nr.intersect(Rect::new(0, 0, self.root_w, self.root_h)).is_some() { self.widgets[id.0].draw(win); } return; } } if let Ok(children) = self.taffy.child_ids(node) { for child in children { self.draw_node(child, win); } } }
    fn draw_scrollbars(&self, win: &mut Window) { for i in 0..self.scrolls.len() { if self.scrolls[i].content_height <= self.scrolls[i].viewport_height { continue; } if let Ok(v) = self.taffy.layout(self.scroll_nodes[i]) { let x = v.location.x as i32 + v.size.width as i32 - 10; let y = v.location.y as i32; let h = v.size.height as i32; let thumb_h = ((h as f32) * (self.scrolls[i].viewport_height / self.scrolls[i].content_height)).max(12.0) as i32; let travel = (h - thumb_h).max(0); let top = y + if self.scrolls[i].max_offset() <= 0.0 { 0 } else { ((self.scrolls[i].offset_y / self.scrolls[i].max_offset()) * travel as f32) as i32 }; let _ = Rectangle::new(Point::new(x, y), Size::new(10, h as u32)).into_styled(PrimitiveStyle::with_fill(Rgb888::new(SCROLLBAR_BG.0, SCROLLBAR_BG.1, SCROLLBAR_BG.2))).draw(win); let _ = Rectangle::new(Point::new(x, top), Size::new(10, thumb_h as u32)).into_styled(PrimitiveStyle::with_fill(Rgb888::new(SCROLLBAR_THUMB.0, SCROLLBAR_THUMB.1, SCROLLBAR_THUMB.2))).draw(win); } } }
}

fn constraints_from_taffy(known: TSize<Option<f32>>, available: TSize<AvailableSpace>) -> Constraints { fn max(v: AvailableSpace) -> f32 { match v { AvailableSpace::Definite(v) => v, AvailableSpace::MinContent => 0.0, AvailableSpace::MaxContent => f32::INFINITY } } Constraints { min_width: known.width.unwrap_or(0.0), max_width: max(available.width), min_height: known.height.unwrap_or(0.0), max_height: max(available.height) } }

impl Default for Ui { fn default() -> Self { Self::new() } }

impl Rect { fn from_taffy(l: &taffy::tree::Layout) -> Self { Self::new(l.location.x.round() as i32, l.location.y.round() as i32, l.size.width.max(0.0).round() as u32, l.size.height.max(0.0).round() as u32) } }
