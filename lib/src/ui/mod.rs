...
//!
//! Containers own layout. Leaf controls implement [`Widget`].
...
impl Ui {
    ...
    fn container(&mut self, parent: NodeId, direction: FlexDirection) -> NodeId {
        ...
        self.taffy.add_child(parent, n).expect("taffy: add container");
        self.needs_layout = true;
        n
    }
    ...
    pub fn panel(&mut self, parent: NodeId) -> NodeId {
        ...
        let n = self.taffy.new_leaf_with_context(s, NodeCtx::Panel).expect("taffy: create panel");
    ...
    pub fn scroll_by(&mut self, scroll: ScrollViewId, dy: f32) -> bool {
        let changed = if let Some(s) = self.scrolls.get_mut(scroll.0) {
            let old = s.offset_y;
            s.set_offset(old + dy);
            old != s.offset_y
        } else { false };
    ...
    pub fn scroll_to_bottom(&mut self, scroll: ScrollViewId) -> bool {
        self.scroll_max_offset(scroll).map(|y| self.scroll_to(scroll, y)).unwrap_or(false)
    }
    ...
    pub fn button(&mut self, parent: NodeId, label: &str) -> WidgetId {
        ...
        s.padding = taffy::geometry::Rect {
            left: LP::length(12.0), right: LP::length(12.0),
            top: LP::length(5.0), bottom: LP::length(5.0),
        };
    ...
    pub fn text_input_with(&mut self, parent: NodeId, text: &str) -> WidgetId {
        let mut s = Style::default();
        s.align_items = Some(AlignItems::CENTER);
        s.min_size = TSize { width: LPA::auto(), height: LPA::length(26.0) };
        s.padding = taffy::geometry::Rect {
            left: LP::length(6.0), right: LP::length(6.0),
            top: LP::length(4.0), bottom: LP::length(4.0),
        };
    ...
    pub fn button_mut(&mut self, id: WidgetId) -> Option<&mut Button> {
        self.widgets.get_mut(id.0)?.as_any_mut().downcast_mut::<Button>()
    }
    pub fn label_mut(&mut self, id: WidgetId) -> Option<&mut Label> {
        self.widgets.get_mut(id.0)?.as_any_mut().downcast_mut::<Label>()
    }
    pub fn text_input_mut(&mut self, id: WidgetId) -> Option<&mut TextInput> {
        self.widgets.get_mut(id.0)?.as_any_mut().downcast_mut::<TextInput>()
    }
    pub fn text(&self, id: WidgetId) -> Option<&str> {
        let w = self.widgets.get(id.0)?;
        w.as_any().downcast_ref::<TextInput>().map(|v| v.text())
            .or_else(|| w.as_any().downcast_ref::<Label>().map(|v| v.text()))
    }
    pub fn set_label(&mut self, id: WidgetId, text: &str) {
        if let Some(v) = self.label_mut(id) { v.set_text(text); self.needs_layout = true; }
    }
    ...
    pub fn set_root_size(&mut self, w: u32, h: u32) {
        ...
        let _ = self.style(self.root, |s| {
            s.size = TSize { width: Dimension::length(w as f32), height: Dimension::length(h as f32) }
        });
    }

    fn compute(&mut self) {
        let root = self.root;
        let rw = self.root_w; let rh = self.root_h;
    ...
    fn apply_layout(&mut self) {
        for clip in &mut self.widget_clips { *clip = None; }
        for rect in &mut self.scroll_rects { *rect = Rect::default(); }
        self.apply_layout_node(self.root, 0.0, 0.0, None);
    }

    fn apply_layout_node(&mut self, node: NodeId, parent_x: f32, parent_y: f32, inherited_clip: Option<Rect>) {
        let Ok(layout) = self.taffy.layout(node).copied() else { return; };
        let ctx = self.taffy.get_node_context(node).copied();
        let x = parent_x + layout.location.x;
        let mut y = parent_y + layout.location.y;
        let rect = Rect::new(x.round() as i32, y.round() as i32, layout.size.width.max(0.0).round() as u32, layout.size.height.max(0.0).round() as u32);

        let mut clip = inherited_clip;
        if let Some(NodeCtx::ScrollView(sid)) = ctx {
            self.scroll_rects[sid.0] = rect;
            clip = match clip { Some(existing) => existing.intersect(rect), None => Some(rect) };
        }
        ...
        if let Some(NodeCtx::Widget(id)) = ctx {
            let widget_rect = Rect::new(x.round() as i32, y.round() as i32, layout.size.width.max(0.0).round() as u32, layout.size.height.max(0.0).round() as u32);
        ...
        match event {
            UiEvent::Down { x, y } => {
                if let Some((sid, grab)) = self.scrollbar_at(*x, *y) {
                    if let Some(s) = self.scrolls.get_mut(sid.0) { s.dragging = true; s.drag_grab = grab; }
                    return true;
                }
            ...
            UiEvent::Move { x, y } => {
                ...
                for i in 0..self.scrolls.len() {
                    ...
                    let (thumb_top, thumb_h) = self.scrollbar_geometry(sid, v);
                    let track_h = v.h as f32;
                    let rel = (*y as f32 - v.y as f32 - self.scrolls[i].drag_grab)
                        .max(0.0).min((track_h - thumb_h).max(0.0));
                    let max = self.scrolls[i].max_offset();
                    let off = if track_h <= thumb_h { 0.0 } else { rel / (track_h - thumb_h) * max };
            ...
            UiEvent::Up { x, y } => {
                let mut changed = false;
                for s in &mut self.scrolls { if s.dragging { changed = true; } s.dragging = false; }
    ...
    fn scrollbar_geometry(&self, sid: ScrollViewId, r: Rect) -> (f32, f32) {
        let s = self.scrolls[sid.0];
        let h = r.h as f32;
        if h <= 0.0 || s.content_height <= 0.0 { return (0.0, h); }
        let thumb_h = (h * (s.viewport_height / s.content_height.max(s.viewport_height))).max(12.0).min(h);
        let travel = (h - thumb_h).max(0.0);
        let top = if s.max_offset() <= 0.0 { 0.0 } else { (s.offset_y / s.max_offset()) * travel };
        (top, thumb_h)
    }
    ...
    fn scrollbar_at(&self, x: i32, y: i32) -> Option<(ScrollViewId, f32)> {
        for i in (0..self.scrolls.len()).rev() {
            ...
            if self.scrolls[i].content_height <= self.scrolls[i].viewport_height || r.h == 0 { continue; }
            let xr = r.x + r.w as i32 - 10;
            if x < xr || x > r.x + r.w as i32 || y < r.y || y > r.y + r.h as i32 { continue; }
            let sid = ScrollViewId(i);
            let (thumb_top, thumb_h) = self.scrollbar_geometry(sid, r);
            let local = (y - r.y) as f32;
            // Preserve the exact point inside the thumb where the user grabbed it.
            let grab = (local - thumb_top).max(0.0).min(thumb_h);
            return Some((sid, grab));
        }
        None
    }
    ...
    fn draw_scrollbars(&self, win: &mut Window) {
        for i in 0..self.scrolls.len() {
            if self.scrolls[i].content_height <= self.scrolls[i].viewport_height { continue; }
            let r = self.scroll_rects[i];
            if r.w == 0 || r.h == 0 { continue; }
            let x = r.x + r.w as i32 - 10;
            let y = r.y;
            let h = r.h;
            let (thumb_top, thumb_h) = self.scrollbar_geometry(ScrollViewId(i), r);
            let _ = Rectangle::new(Point::new(x, y), Size::new(10, h))
                .into_styled(PrimitiveStyle::with_fill(SCROLLBAR_BG)).draw(win);
            let _ = Rectangle::new(Point::new(x, y + thumb_top.round() as i32), Size::new(10, thumb_h.round() as u32))
                .into_styled(PrimitiveStyle::with_fill(SCROLLBAR_THUMB)).draw(win);
        }
    }
}

fn constraints_from_taffy(known: TSize<Option<f32>>, available: TSize<AvailableSpace>) -> Constraints {
...
impl Rect {
    fn from_taffy(l: &taffy::tree::Layout) -> Self {
        Self::new(l.location.x.round() as i32, l.location.y.round() as i32,
            l.size.width.max(0.0).round() as u32, l.size.height.max(0.0).round() as u32)
    }
}
