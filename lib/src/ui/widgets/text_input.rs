...
impl Widget for TextInput {
    fn measure(&self, c: Constraints) -> Size<f32> { c.clamp(Size { width: self.text.chars().count().max(8) as f32 * FONT_W + 12.0, height: 26.0 }) }
    ...
    fn draw(&self, win: &mut crate::wm::Window) {
        ...
        if self.focused {
            ...
            let _ = Rectangle::new(Point::new(cx, r.y + 4), Size::new(2, r.h.saturating_sub(8))).into_styled(PrimitiveStyle::with_fill(TEXT)).draw(win);
        }
    }
