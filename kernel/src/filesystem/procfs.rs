...
impl ProcFs {
    ...
    fn pid_inode(pid: i32, kind: u32) -> Option<u32> {
        ...
        PID_BASE.checked_add((pid as u32).checked_mul(PID_STRIDE)?)?.checked_add(kind)
    }

    fn decode_pid_inode(inode: u32) -> Option<(i32, u32)> {
        if inode < PID_BASE { return None; }
        let rel = inode - PID_BASE;
        Some(((rel / PID_STRIDE) as i32, rel % PID_STRIDE))
    }

    fn state_char(task: &Task) -> char {
        if task.zombie { 'Z' } else if task.stopped { 'T' } else if task.running { 'R' } else { 'S' }
    }
    ...
    fn data_for_inode(inode: u32) -> Option<Vec<u8>> {
        match inode {
            INO_UPTIME => {
                ...
                Some(format!("{}.{:03} {}.{:03}\n", ms / 1000, ms % 1000, ms / 1000, ms % 1000).into_bytes())
            }
    ...
    fn parse_path(path: &str) -> Option<u32> {
        ...
        if clean.is_empty() || clean == "." { return Some(INO_ROOT); }
        if clean == "uptime" { return Some(INO_UPTIME); }
        if clean == "mounts" { return Some(INO_MOUNTS); }
...
impl Filesystem for ProcFs {
        ...
        if let Some((pid, kind)) = Self::decode_pid_inode(inode) {
            if kind == 0 && Self::task_by_pid(pid).is_some() {
                return Some(alloc::vec![
                    DirEntry { inode: Self::pid_inode(pid, 1)?, name: "status".to_string(), file_type: 1, size: 0 },
                    DirEntry { inode: Self::pid_inode(pid, 2)?, name: "cwd".to_string(), file_type: 1, size: 0 },
                    DirEntry { inode: Self::pid_inode(pid, 3)?, name: "stat".to_string(), file_type: 1, size: 0 },
                ]);
            }
        }
        None
    ...
    fn resolve_path(&self, path: &str) -> Option<u32> { Self::parse_path(path) }

    fn read_at(&self, inode: u32, offset: u64, buf: &mut [u8]) -> usize {
        let Some(data) = Self::data_for_inode(inode) else { return 0; };
    ...
    fn write_at(&mut self, _inode: u32, _offset: u64, _buf: &[u8]) -> usize { 0 }
