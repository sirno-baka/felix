...
pub static IDE: KMutex<IDEController> = KMutex::<IDEController>::new(IDEController::new());
...
impl IDEController {
    ...
    pub unsafe fn initialize(&mut self) -> Result<(), u8> {
        unsafe {
            outb(0x3F6, 2);
            inb(0x3F6);
        }
        ...
        // println!("secondary");
        let mut secondary = IDEChannelRegisters::new(ATAChannel::Secondary, 0x170, 0x374, 0, 0);
        // println!("channels");
        let channels: [Arcm<IDEChannelRegisters>; 2] = [Arcm::new(primary), Arcm::new(secondary)];
        ...
        for i in 0..2 {
            for j in 0..2 {
                ...
                if err != 0 {
                    ...
                    channels[i]
                        .lock()
                        .write(ATAReg::COMMAND, ATACommand::IdentifyPacket as u8);
                    sleep(1);
                }
                ...
                self.devices[count].signature = u16::from_le_bytes(
                    ide_buf[ATAIdentify::DEVICETYPE..ATAIdentify::DEVICETYPE + size_of::<u16>()]
                        .try_into()
                        .unwrap(),
                );
                self.devices[count].capabilities = u16::from_le_bytes(
                    ide_buf
                        [ATAIdentify::CAPABILITIES..ATAIdentify::CAPABILITIES + size_of::<u16>()]
                        .try_into()
                        .unwrap(),
                );
                self.devices[count].command_sets = u32::from_le_bytes(
                    ide_buf[ATAIdentify::COMMANDSETS..ATAIdentify::COMMANDSETS + size_of::<u32>()]
                        .try_into()
                        .unwrap(),
                );
                ...
                if self.devices[count].r#type == IDEType::ATA as u16 {
                    ...
                    if (self.devices[count].command_sets & (1 << 26)) != 0 {
                        ...
                        self.devices[count].size = u32::from_le_bytes(
                            ide_buf[ATAIdentify::MAX_LBA..ATAIdentify::MAX_LBA + size_of::<u32>()]
                                .try_into()
                                .unwrap(),
                        );
                    }
                ...
                for k in (0..40).step_by(2) {
                    self.devices[count].model[k] = ide_buf[ATAIdentify::MODEL as usize + k + 1];
                    self.devices[count].model[k + 1] = ide_buf[ATAIdentify::MODEL as usize + k];
                }
    ...
    fn read_buffer(channel: &mut IDEChannelRegisters, reg: u8, buffer: &mut [u32], quads: u32) {
        channel.read_buffer(reg, buffer, quads);
    }

    fn polling(channel: &mut IDEChannelRegisters, advanced_check: u32) -> Result<(), u8> {
    ...
    pub fn read_sectors(&mut self, drive: u8, numsects: u8, lba: u32, edi: u32) -> Result<(), u8> {
        ...
        } else if (lba + numsects as u32 > device.size) && (device.r#type == IDEType::ATA as u16) {
            // Seeking to invalid position
            return Err(0x2);
        // 3- Read in PIO Mode through Polling & IRQs
        } else {
            if device.r#type == IDEType::ATA as u16 {
                match ATA::access(ATADirection::Read as u8, device, lba, numsects, edi) {
                    Ok(_) => {}
    ...
    /// + edi: adress of the buffer we want to fill
    pub fn write_sectors(&mut self, drive: u8, numsects: u8, lba: u32, edi: u32) -> Result<(), u8> {
        let device: &mut IDEDevice = &mut self.devices[drive as usize];
        ...
        // 3- Read in PIO Mode through Polling & IRQs
            ...
            if device.r#type == IDEType::ATA as u16 {
                match ATA::access(ATADirection::Write as u8, device, lba, numsects, edi) {
                    Ok(_) => {}
                    Err(err) => return Err(device.print_error(err)),
                }
            } else if device.r#type == IDEType::ATAPI as u16 {
