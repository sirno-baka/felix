...
impl IDEDevice {
    ...
    pub fn print_error(&self, mut err: u8) -> u8 {
        ...
        let binding = match &self.channel {
            Some(x) => x,
            None => {
                println!("- Channel non-initialized");
                return 23;
            }
        };
    ...
    pub fn read_sectors(&self, numsects: u8, lba: u32, edi: u32) -> Result<(), u8> {
        // 1- Check if the drive presents
        if self.reserved == 0 {
            // Drive not found
            return Err(0x1);
        // 2- Check if inputs are valid
        } else if (lba + numsects as u32 > self.size) && (self.r#type == IDEType::ATA as u16) {
            // Seeking to invalid position
            return Err(0x2);
        // 3- Read in PIO Mode through Polling & IRQs
        } else {
            if self.r#type == IDEType::ATA as u16 {
                match ATA::access(ATADirection::Read as u8, self, lba, numsects, edi) {
                    Ok(_) => {}
                    Err(err) => return Err(self.print_error(err)),
                }
            } else if self.r#type == IDEType::ATAPI as u16 {
                for i in 0..numsects {
                    match ATAPI::read(self, lba + i as u32, 1, edi + i as u32 * atapi::SECTOR_SIZE)
                    {
                        Ok(_) => {}
                        Err(err) => return Err(self.print_error(err)),
                    }
    ...
    /// + edi: adress of the buffer we want to fill
    pub fn write_sectors(&mut self, numsects: u8, lba: u32, edi: u32) -> Result<(), u8> {
        // 1- Check if the drive presents
        if self.reserved == 0 {
            // Drive not found
            return Err(0x1);
        ...
        // 3- Read in PIO Mode through Polling & IRQs
            ...
            if self.r#type == IDEType::ATA as u16 {
                match ATA::access(ATADirection::Write as u8, self, lba, numsects, edi) {
                    Ok(_) => {}
                    Err(err) => return Err(self.print_error(err)),
                }
            } else if self.r#type == IDEType::ATAPI as u16 {
...
impl BlockDevice for IDEDevice {
    fn read_sectors(&self, numsects: u8, lba: u32, buf: u32) -> Result<(), u8> {
        IDEDevice::read_sectors(self, numsects, lba, buf)
    }

    fn write_sectors(&mut self, numsects: u8, lba: u32, buf: u32) -> Result<(), u8> {
        IDEDevice::write_sectors(self, numsects, lba, buf)
    }
