...
impl RicohR5c475 {
    ...
    pub fn pc16(&self) -> Pc16 {
        Pc16::new(self.bar0_virt)
    }
    ...
    pub unsafe fn dump_cardbus_socket(&self) {
        unsafe {
            ...
            crate::println!(
                "[PCMCIA]   +00 SOCKET_EVENT   = {:08x}",
                self.mmio_read32(CB_SOCKET_EVENT)
            );
            crate::println!(
                "[PCMCIA]   +04 SOCKET_MASK    = {:08x}",
                self.mmio_read32(CB_SOCKET_MASK)
            );
            crate::println!(
                "[PCMCIA]   +08 SOCKET_STATE   = {:08x}",
                self.mmio_read32(CB_SOCKET_STATE)
            );
            crate::println!(
                "[PCMCIA]   +0c SOCKET_FORCE   = {:08x}",
                self.mmio_read32(CB_SOCKET_FORCE)
            );
            crate::println!(
                "[PCMCIA]   +10 SOCKET_CONTROL = {:08x}",
                self.mmio_read32(CB_SOCKET_CONTROL)
            );
            crate::println!(
                "[PCMCIA]   +20 SOCKET_POWER   = {:08x}",
                self.mmio_read32(CB_SOCKET_POWER)
            );
        }
    }
    ...
    pub fn dump_pci_config(&self) {
        ...
        crate::println!(
            "[PCMCIA] PCI command     = {:04x}",
            self.pci.read_u16(PCI_COMMAND)
        );
        crate::println!(
            "[PCMCIA] IRQ line/pin    = {:02x}/{:02x}",
            self.pci.read_u8(PCI_INTERRUPT_LINE),
            self.pci.read_u8(PCI_INTERRUPT_PIN)
        );
        crate::println!(
            "[PCMCIA] Bridge control  = {:04x}",
            self.pci.read_u16(PCI_BRIDGE_CONTROL)
        );
        crate::println!(
            "[PCMCIA] Ricoh misc 0x82 = {:04x}",
            self.pci.read_u16(PCI_RICOH_MISC_CONTROL)
        );
    }

    pub fn dump(&self) {
        unsafe {
            self.dump_cardbus_socket();
        }
        unsafe {
            self.pc16().dump();
        }
...
impl SocketController for RicohR5c475 {
    ...
    fn restore_host_decode(&self) {
        ...
        self.pci
            .write_u32(PCI_CB_IO_BASE_0, (CF_IO_BASE as u32) & 0xffff_fffc);
        self.pci
            .write_u32(PCI_CB_IO_LIMIT_0, (CF_IO_END as u32) | 0x3);
    }

    fn pc16(&self) -> Pc16 {
        RicohR5c475::pc16(self)
    }
    unsafe fn enable_csc(&self, irq: u8) {
        RicohR5c475::enable_csc(self, irq);
    }
    unsafe fn ack_csc(&self) -> (u8, u32, bool) {
        RicohR5c475::ack_csc(self)
    }
}
