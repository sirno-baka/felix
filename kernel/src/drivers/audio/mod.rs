...
impl Stream {
    const fn new() -> Self {
        Self { owner: -1, volume: 256, read: 0, write: 0, len: 0, data: [0; STREAM_SAMPLES] }
    }
...
impl Mixer {
    ...
    fn stream_for(&mut self, owner: usize) -> Option<&mut Stream> {
        ...
        let idx = self.streams.iter().position(|s| s.owner < 0)
    ...
    pub fn set_volume(&mut self, owner: usize, volume: u16) -> bool {
        let owner_i = owner.min(i16::MAX as usize) as i16;
        let Some(stream) = self.streams.iter_mut().find(|s| s.owner == owner_i) else { return false; };
...
pub fn init() {
    ...
    let irq_enabled = irq_result.is_ok();
    backend.set_irq_enabled(irq_enabled);
